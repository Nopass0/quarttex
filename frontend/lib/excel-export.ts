...
export function exportTransactionsToExcel(
  ...
  const excelData = transactions.map((transaction) => {
    ...
    if (!countInRubEquivalent) {
      baseData['Курс'] = transaction.effectiveRate != null ? toTwoDecimals(transaction.effectiveRate) : null
      baseData['Курс пересчитан'] = transaction.isRecalculated ? 'Да' : 'Нет'
      baseData['Сумма (USDT)'] = usdtAmount != null ? toTwoDecimals(usdtAmount) : null
    }
...
export function exportPayoutsToExcel(
  ...
  const excelData = payouts.map((payout) => ({
    ...
    'Сумма (USDT)': payout.amountUsdt != null ? toTwoDecimals(payout.amountUsdt) : null,
    'Комиссия (%)': payout.feePercent ?? null,
    ...
    'Трейдер': payout.trader ? `#${payout.trader.numericId} ${payout.trader.email}` : '',
    'Дата создания': formatDateTime(payout.createdAt),
    'Дата принятия': payout.acceptedAt ? formatDateTime(payout.acceptedAt) : '',
    'Дата подтверждения': payout.confirmedAt ? formatDateTime(payout.confirmedAt) : '',
    'Дата отмены': payout.cancelledAt ? formatDateTime(payout.cancelledAt) : '',
  }))
