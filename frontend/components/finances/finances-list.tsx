...
export function FinancesList() {
  ...
  const getStatusBadge = (status: string) => {
    const statusConfig: any = {
      ...
      READY: { label: "Готово", color: "bg-[#530FAD]/10 text-[#530FAD] border-[#530FAD]/20 dark:bg-purple-800/60/20 dark:text-[#7c3aed] dark:border-[#7c3aed]/30" },
      ...
      CANCELED: { label: "Отменено", color: "bg-purple-50/30 text-gray-600 border-purple-200/60 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-700" },
    }
  ...
  const getTypeBadge = (type: string) => {
    return type === "IN" ? "bg-[#530FAD]/10 text-[#530FAD] dark:bg-purple-800/60/20 dark:text-[#7c3aed]" : "bg-red-50 text-red-600 dark:bg-red-900/30 dark:text-red-400"
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="bg-purple-50/10 p-6 dark:bg-purple-800/60 dark:border-gray-700">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm text-gray-600 dark:text-gray-400">БАЛАНС КОШЕЛЬКА</span>
            <Button 
              variant="link" 
              className="text-[#530FAD] dark:text-[#7c3aed] p-0 h-auto text-sm"
        ...
        <Card className="bg-purple-50/10 p-6 dark:bg-purple-800/60 dark:border-gray-700">
          ...
          <div className="text-2xl font-bold text-[#530FAD] dark:text-[#7c3aed]">+{financeStats.totalProfit.toLocaleString('ru-RU')} {financeStats.currency}</div>
        </Card>
      </div>
      
      {/* Filters */}
      <Card className="bg-purple-50/10 p-4 dark:bg-purple-800/60 dark:border-gray-700">
        <div className="flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center gap-4">
          <div className="flex-1 min-w-[200px]">
            <div className="relative">
              <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-[#530FAD] dark:text-[#7c3aed] h-4 w-4" />
              <Input
          ...
          <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
            ...
            <Select value={dateFilter} onValueChange={setDateFilter}>
              <SelectTrigger className="w-full sm:w-[180px] dark:bg-[#0f0f0f] dark:border-gray-600 dark:text-[#eeeeee]">
                <Calendar className="mr-2 h-4 w-4 text-[#530FAD] dark:text-[#7c3aed]" />
          ...
          <div className="flex gap-2 w-full sm:w-auto">
            <Button variant="outline" className="flex-1 sm:flex-initial dark:border-gray-600 dark:text-gray-300 dark:hover:bg-purple-800/60/50">
              <Filter className="mr-2 h-4 w-4 text-[#530FAD] dark:text-[#7c3aed]" />
              <span className="hidden sm:inline">Фильтры</span>
            </Button>
            
            <Button variant="outline" className="flex-1 sm:flex-initial dark:border-gray-600 dark:text-gray-300 dark:hover:bg-purple-800/60/50">
              <Download className="mr-2 h-4 w-4 text-[#530FAD] dark:text-[#7c3aed]" />
      ...
      {/* Transactions Table */}
      <Card className="bg-purple-50/10 overflow-hidden dark:bg-purple-800/60 dark:border-gray-700">
        <Table>
          <TableHeader>
            <TableRow className="bg-gray-50/50 hover:bg-purple-50/50/50 dark:bg-[#0f0f0f] dark:hover:bg-[#0f0f0f]">
          ...
          <TableBody>
            {filteredTransactions.length === 0 ? (
              ...
              filteredTransactions.map((transaction) => {
                ...
                return (
                  <TableRow key={transaction.id} className="hover:bg-purple-50/50/50 dark:hover:bg-purple-800/60/30">
                      ...
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button
                            variant="ghost"
                            size="sm"
                            className="h-8 w-8 p-0 dark:hover:bg-purple-800/60/50"
                          >
                            <MoreVertical className="h-4 w-4 text-[#530FAD] dark:text-[#7c3aed]" />
                          </Button>
                        </DropdownMenuTrigger>
                        <DropdownMenuContent align="end" className="w-48 dark:bg-purple-800/60 dark:border-gray-700">
                          <DropdownMenuItem className="dark:hover:bg-purple-800/60/50">
                            <Eye className="mr-2 h-4 w-4 text-[#530FAD] dark:text-[#7c3aed]" />
                            Подробнее
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
            ...
            {!isAddingNewWallet ? (
              <>
                ...
                <Button 
                  ...
                  className="bg-[#530FAD] hover:bg-[#530FAD]/90 dark:bg-purple-800/60 dark:hover:bg-purple-800/60/90"
